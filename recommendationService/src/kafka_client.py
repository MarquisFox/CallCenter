...
class KafkaService:
    ...
    def _get_retry_key(self, call_id: str) -> str:
        return f"retry:gigachat:{self.consumer.topics}:{call_id}"
                ...
                for msg in records:
                    ...
                    try:
                        ...
                        if attempt >= config.MAX_RETRIES:
                            # Пропускаем, уже должно быть в DLQ
                            self.consumer.commit()
                            continue

                        input_data = InputMessage(**msg.value)
                        output_message = processor_func(input_data)
                        self.producer.send(
                            self.output_topic,
                            key=output_message.callId,
                        ...
                        if attempt <= config.MAX_RETRIES:
                            ...
                            time.sleep(backoff)
                            # Не коммитим – сообщение останется в партиции
                        else:
